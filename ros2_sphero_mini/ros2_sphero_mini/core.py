...
from ros2_sphero_mini._constants import *
import struct
import time
import sys
import logging #standard library replacement for ros_1 rospy
...
class SpheroMini():
    def __init__(self, MACAddr, verbosity = 4, user_delegate = None):
        ...
        if self.verbosity > 0:
            logger.debug("[INFO] Connecting to %s", MACAddr)
        ...
        if self.verbosity > 1:
            logger.debug("[INIT] Read all characteristics and descriptors")
        ...
        if self.verbosity > 1:
            logger.debug("[INIT] Writing AntiDOS characteristic unlock code")
        self.AntiDOS_characteristic.write("usetheforce...band".encode(), withResponse=True)

        # Enable DFU notifications:
        if self.verbosity > 1:
            logger.debug("[INIT] Configuring DFU descriptor")
        ...
        if self.verbosity > 1:
            logger.debug("[INIT] Reading DFU2 characteristic")
        _ = self.DFU2_characteristic.read()

        # Enable API notifications:
        if self.verbosity > 1:
            logger.debug("[INIT] Configuring API dectriptor")
        ...
        if self.verbosity > 1:
            logger.debug("[INIT] Initialization complete\n")
    ...
    def wake(self):
        ...
        '''
        if self.verbosity > 2:
            logger.debug("[SEND {}] Waking".format(self.sequence))
    ...
    def sleep(self, deepSleep=False):
        ...
        if deepSleep:
            ...
            if self.verbosity > 0:
                logger.debug("[INFO] Going into deep sleep. Connect USB power to wake.")
        else:
    ...
    def setLEDColor(self, red = None, green = None, blue = None):
        ...
        '''
        if self.verbosity > 2:
            logger.debug("[SEND {}] Setting main LED colour to [{}, {}, {}]".format(self.sequence, red, green, blue))
    ...
    def setBackLEDIntensity(self, brightness=None):
        ...
        '''
        if self.verbosity > 2:
            logger.debug("[SEND {}] Setting backlight intensity to {}".format(self.sequence, brightness))
    ...
    def roll(self, speed=None, heading=None):
        ...
        '''
        if self.verbosity > 2:
            logger.debug("[SEND {}] Rolling with speed {} and heading {}".format(self.sequence, speed, heading))
    
        if abs(speed) > 255:
            logger.debug("WARNING: roll speed parameter outside of allowed range (-255 to +255)")
    ...
    def resetHeading(self):
        ...
        '''
        if self.verbosity > 2:
            logger.debug("[SEND {}] Resetting heading".format(self.sequence))
    ...
    def returnMainApplicationVersion(self):
        ...
        '''
        if self.verbosity > 2:
            logger.debug("[SEND {}] Requesting firmware version".format(self.sequence))
    ...
    def getBatteryVoltage(self):
        ...
        '''
        if self.verbosity > 2:
            logger.debug("[SEND {}] Requesting battery voltage".format(self.sequence))
    ...
    def stabilization(self, stab = True):
        ...
        if stab == True:
            if self.verbosity > 2:
                    logger.debug("[SEND {}] Enabling stabilization".format(self.sequence))
            val = 1
        else:
            if self.verbosity > 2:
                    logger.debug("[SEND {}] Disabling stabilization".format(self.sequence))
            val = 0
        self._send(self.API_V2_characteristic,
                   devID=deviceID['driving'],
    ...
    def getAcknowledgement(self, ack):
        ...
        while(1):
            ...
            if self.sphero_delegate.notification_seq == self.sequence-1: # use one less than sequence, because _send function increments it for next send. 
                if self.verbosity > 3:
                    logger.debug("[RESP {}] {}".format(self.sequence-1, self.sphero_delegate.notification_ack))
                self.sphero_delegate.clear_notification()
                break
            elif self.sphero_delegate.notification_seq >= 0:
                logger.debug("Unexpected ACK. Expected: {}/{}, received: {}/{}".format(
                    ack, self.sequence, self.sphero_delegate.notification_ack.split()[0],
                    self.sphero_delegate.notification_seq)
                    )
            if time.time() > start + 10:
                logger.debug("Timeout waiting for acknowledgement: {}/{}".format(ack, self.sequence))
                break
...
# =======================================================================
    ...
    def configureCollisionDetection(self,
        ...
        if self.verbosity > 2:
            logger.debug("[SEND {}] Configuring collision detection".format(self.sequence))
    ...
    def configureSensorStream(self): # Use default values
        ...
        if self.verbosity > 2:
            logger.debug("[SEND {}] Configuring sensor stream".format(self.sequence))
    ...
    def configureSensorMask(self,
        ...
        if self.verbosity > 2:
            logger.debug("[SEND {}] Configuring sensor mask".format(self.sequence))
        ...
        e.g. logger.debug(sphero.IMU_yaw) # displays the current yaw angle
        '''
...
class MyDelegate(btle.DefaultDelegate):
    ...
    def handleNotification(self, cHandle, data):
        ...
        # logger.debug("Received notification with packet %s", str(data))
        ...
        for data_byte in data: # parse each byte separately (sometimes they arrive simultaneously)
            ...
            if data_byte == sendPacketConstants['EndOfPacket']:
                ...
                try:
                    ...
                    logger.debug("Warning: notification packet unparseable %s", self.notificationPacket)
                ...
                if checksum != chsum: # check computed checksum against that recieved in the packet
                    logger.debug("Warning: notification packet checksum failed - %s", str(self.notificationPacket))
                    self.notificationPacket = [] # Discard this packet
                    return # exit
                ...
                if flags_bits & flags['isResponse']: # it is a response
                    ...
                    if devid == deviceID['powerInfo'] and commcode == powerCommandIDs['wake']:
                        ...
                        self.notification_ack = "Unknown acknowledgement" #logger.debug(self.notificationPacket)
                        logger.debug(self.notificationPacket, "===================> Unknown ack packet")
                    ...
                    if devid == deviceID['sensor'] and commcode == sensorCommands['collisionDetectedAsync']:
                        ...
                        logger.debug("\tAxis: %s", dir)
                        logger.debug("\tX_mag: %s", X_mag)
                        logger.debug("\tY_mag: %s", Y_mag)
                    ...
                    # Sensor response:
                        ...
                        for name, value in zip(self.sphero_class.configured_sensors, nums):
                            logger.debug("Setting sensor %s at %s.", name, str(value))
                            setattr(self.sphero_class, name, value)
                        
                    # Unrecognized packet structure:
                    else:
                        self.notification_ack = "Unknown asynchronous notification" #logger.debug(self.notificationPacket)
                        logger.debug(str(self.notificationPacket) + " ===================> Unknown async packet")
